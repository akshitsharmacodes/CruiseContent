  ...
  return (
    <motion.div whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} className="w-full">
      <Button 
        ...
        disabled={loading}
        variant="outline"
        className="w-full flex items-center justify-center gap-3 h-11 bg-card hover:bg-muted border-border text-foreground transition-all rounded-lg"
      >
        {loading ? (
        ...
        <span className="font-medium text-sm">{loading ? 'Redirecting...' : label}</span>
      </Button>
    </motion.div>
  );
};
